 ...
 * @author Hoshiiro
 */
class PLVMediaPlayerMoreSubtitleSettingLayoutPort @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = 0
) : FrameLayout(context, attrs, defStyleAttr), View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    private val moreSubtitleSettingBackIv by lazy { findViewById<ImageView>(R.id.plv_media_player_more_subtitle_setting_back_iv) }
    ...
    init {
        LayoutInflater.from(context)
            .inflate(R.layout.plv_media_player_ui_component_more_subtitle_setting_layout_port, this)
    ...
    override fun onClick(v: View) {
        when (v.id) {
            moreSubtitleSettingBackIv.id -> backToMoreLayout()
            this.id -> closeLayout()
        }
    }
    ...
    private class SubtitleItemLayout(
        ...
        init {
            LayoutInflater.from(context).inflate(R.layout.plv_media_player_ui_component_more_subtitle_item_port, this)
